...
if C.zzz.Threat ~= true then return end
...
threat:UnregisterEvent("PLAYER_ENTERING_WORLD")
	...
	-- config
	cfg.Font = GetLocale() == "zhCN" and "Fonts\\ARKai_T.ttf" or "Fonts\\bLEI00D.ttf"
	cfg.Pos = {"BOTTOMLEFT", UIParent, "BOTTOMLEFT", 280, 14}	-- 位置(可锚在包括oUF头像的任意框体上)
	cfg.NameTextL = 3										-- 名字长度 单位:字
	...
	ThreatFrame:SetWidth(cfg.Width)  --主框体宽度
	ThreatFrame:SetHeight(cfg.Height)
	...
	Texture:SetHeight(cfg.Height)
	Texture:SetWidth(cfg.Width)	
	...
	local function UpdateThreatFlag()
		...
		for key, value in ipairs(ThreatList) do
			if ThreatList[key].isTanking then
				...
				if not FlagT then
					...
					FlagT.Name:SetHeight(8)
					FlagT.Name:SetWidth(8)
					FlagT.Name:SetTexture(cfg.ArrowLarge)
					FlagT.Name:SetPoint("BOTTOM", FlagT, "TOP", 0, cfg.Height)
					
					FlagT.Text = FlagT:CreateFontString(nil,"OVERLAY")
					FlagT.Text:SetFont(cfg.Font,8,"THINOUTLINE")
					FlagT.Text:SetPoint("BOTTOM", FlagT.Name, "TOP", 0, -2)
				...
				FlagT:SetPoint("LEFT", ThreatFrame, "LEFT", (cfg.Width+20)*100/130+3, 0)
				FlagT:Show()
		...
		for key, value in ipairs(ThreatList) do
			...
			if not Flag then
				...
				Flag.Name:SetTexture(cfg.ArrowLarge)
				Flag.Name:SetPoint("BOTTOM", Flag, "TOP", 0, cfg.Height)
					
				Flag.Text = Flag:CreateFontString(nil,"OVERLAY")
				Flag.Text:SetFont(cfg.Font,8,"THINOUTLINE")
				Flag.Text:SetPoint("BOTTOM", Flag.Name, "TOP", 0, -2)
			...
			Flag.Text:SetTextColor(Color.r, Color.g, Color.b)
				...
				Flag:SetPoint("LEFT", ThreatFrame, "LEFT", (cfg.Width+20)*rawPercent/130+3, 0)
			Flag:Show()
		end
	end
